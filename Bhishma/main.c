...
#ifdef servercode
static errorcode_t onoff_response(uint16_t element_index, uint16_t client_addr,
		uint16_t appkey_index) {
	struct mesh_generic_state current, target;
...
static errorcode_t onoff_update(uint16_t element_index) {
	...
	current.kind = mesh_generic_state_on_off;
	current.on_off.on = lightbulb_state.fire_current;

	target.kind = mesh_generic_state_on_off;
	target.on_off.on = lightbulb_state.fire_target;

	return mesh_lib_generic_server_update(MESH_GENERIC_ON_OFF_SERVER_MODEL_ID,
			element_index, &current, &target, 0);
}

static void onoff_request(uint16_t model_id, uint16_t element_index,
	...
	if (client_addr == CLIENT_SERVER_ADDRESS) {
		if (delay_ms) {
			printf("\r\nFIRE DETECTION MESSAGE RECEIVED\r\n");
//			fire_detected = 1;
//			fire_response();
		} else
...
static void onoff_change(uint16_t model_id, uint16_t element_index,
		const struct mesh_generic_state *current,
